...
public final class Localization {
    ...
    public static org.schabi.newpipe.extractor.localization.Localization getPreferredLocalization(
        ...
        final org.schabi.newpipe.extractor.localization.Localization localization =
                org.schabi.newpipe.extractor.localization.Localization
                        .fromLocale(getPreferredLocale(context));
        if (App.DEBUG) {
            Log.d(TAG, "Preferred localization set to: " + localization.getLanguageCode());
        }
        // Show Toast for preferred localization
        new Handler(Looper.getMainLooper()).post(() ->
            Toast.makeText(App.getAppContext(),
                "Preferred localization: " + localization.getLanguageCode(),
    ...
    public static ContentCountry getPreferredContentCountry(@NonNull final Context context) {
        ...
        if (contentCountry.equals(context.getString(R.string.default_localization_key))) {
            country = new ContentCountry(Locale.getDefault().getCountry());
        } else {
            country = new ContentCountry(contentCountry);
        }
        if (App.DEBUG) {
            Log.d(TAG, "Preferred content country set to: " + country.getCountryCode());
        }
        // Show Toast for content country
        new Handler(Looper.getMainLooper()).post(() ->
            Toast.makeText(App.getAppContext(),
                "Content country: " + country.getCountryCode(),
                Toast.LENGTH_SHORT).show()
        );
        return country;
    }
    ...
    public static Locale getPreferredLocale(@NonNull final Context context) {
        final Locale locale = getLocaleFromPrefs(context, R.string.content_language_key);
        if (App.DEBUG) {
            Log.d(TAG, "Preferred locale: " + locale.toLanguageTag());
        }
        return locale;
    }
    ...
    public static Locale getAppLocale(@NonNull final Context context) {
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            final Locale result = Objects.requireNonNullElseGet(customLocale, Locale::getDefault);
            if (App.DEBUG) {
                Log.d(TAG, "App locale (API 33+): " + result.toLanguageTag());
            }
            return result;
        }
        final Locale locale = getLocaleFromPrefs(context, R.string.app_language_key);
        if (App.DEBUG) {
            Log.d(TAG, "App locale: " + locale.toLanguageTag());
        }
        return locale;
    }
    ...
    public static void initPrettyTime(@NonNull final PrettyTime time) {
        ...
        if (App.DEBUG) {
            Log.d(TAG, "PrettyTime initialized");
        }
    }

    public static PrettyTime resolvePrettyTime(@NonNull final Context context) {
        final PrettyTime time = new PrettyTime(getAppLocale(context));
        if (App.DEBUG) {
            Log.d(TAG, "Resolved PrettyTime with locale: " + getAppLocale(context).toLanguageTag());
        }
        return time;
    }
    ...
    public static String relativeTimeOrTextual(@Nullable final Context context,
        ...
        if (parsed == null) {
            ...
            final String result = relativeTime(parsed.offsetDateTime()) + " (" + textual + ")";
            if (App.DEBUG) {
                Log.d(TAG, "Relative time with textual: " + result);
            }
            return result;
        } else {
            final String result = relativeTime(parsed.offsetDateTime());
            if (App.DEBUG) {
                Log.d(TAG, "Relative time: " + result);
            }
            return result;
        }
    }
    ...
    public static void assureCorrectAppLanguage(final Context c) {
        ...
        if (App.DEBUG) {
            Log.d(TAG, "Assured app language: " + getAppLocale(c).toLanguageTag());
        }
    }
            ...
            if (appLanguageValue != null) {
                ...
                if (!appLanguageValue.equals(appLanguageDefaultValue)) {
                    try {
                        ...
                        // Show Toast for successful migration
                        new Handler(Looper.getMainLooper()).post(() ->
                            Toast.makeText(App.getAppContext(),
                                "App language migrated to: " + appLanguageValue,
                                Toast.LENGTH_SHORT).show()
                        );
                        if (App.DEBUG) {
                            Log.d(TAG, "Migrated app language to: " + appLanguageValue);
                        }
                    } catch (final RuntimeException e) {
                        Log.e(TAG, "Failed to migrate previous custom app language "
                                + "setting to public per-app language APIs", e);
                        // Show Toast for migration failure
                        new Handler(Looper.getMainLooper()).post(() ->
                            Toast.makeText(App.getAppContext(),
                                "Failed to migrate app language: " + e.getMessage(),
